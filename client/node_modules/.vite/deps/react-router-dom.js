...
function createRouter(init) {
  ...
  function initialize() {
    ...
    let lazyPromises = lazyMatches.map((m) => loadLazyRouteModule(m.route, detectErrorBoundary2, manifest));
    Promise.all(lazyPromises).then(() => {
      let initialized2 = !state.matches.some((m) => m.route.loader) || init.hydrationData != null;
      if (initialized2) {
        updateState({
          initialized: true
        });
